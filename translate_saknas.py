#!/usr/bin/env python3
"""
Translate Description_sv for rows where Status_Desc_no == SAKNAS.
Saves checkpoint after each batch. Outputs Id, Name, Text3.
"""
...
CHECKPOINT_FILE = "translation_saknas_checkpoint.json"
OUTPUT_FILE     = "Descriptions_NO_saknas_clean.xlsx"
BATCH_SIZE      = 10
...
# ── Load source data ──────────────────────────────────────────────────────────
print("Läser källfil...")
df = pd.read_excel("Norwegian_translations_to_review.xlsx")

saknas = df[df["Status_Desc_no"] == "SAKNAS"].copy()
has_text = saknas["Description_sv"].notna() & (saknas["Description_sv"].astype(str).str.strip() != "")
to_translate = saknas[has_text][["ID", "Name_sv", "Description_sv"]].reset_index(drop=True)
...
for _, row in to_translate.iterrows():
    ...
    if id_str in translations:
        result_rows.append({
            "Id":    row["ID"],
            "Name":  row["Name_sv"],
            "Text3": clean_html(translations[id_str]),
        })
...
if errors:
    print(f"\nMisslyckade batcher: {[e['batch_start'] for e in errors]}")
